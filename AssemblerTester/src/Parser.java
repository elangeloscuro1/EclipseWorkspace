...
/**
 * Parser: Parses each code line into its corresponding parts
 * 
 * @author Angel Tapia
 * @version 1.0
 */
...
public class Parser
{
	...
	/**
	 * DESCRIPTION: Opens input file and prepares it to parse PRECONDITION:
	 * Provide a ASM file POSTCONDITION: Programs ends with a message if file
	 * cannot be opened
	 * 
	 * @param fileName:
	...
	/**
	 * DESCRIPTION: Reads next line from file and parses it into instance
	 * variables PRECONDITION: file stream is opened, called only if
	 * hasMoreCommands() POSTCONDITION: if hasMoreCommands(): current
	 * instruction is parsed into variables
	 */
	...
	/**
	 * DESCRIPTION: Helper method that cleans raw lines instruction by removing
	 * non-essential parts POSTCONDITION: parses into cleanLine variable
	 */
	private void cleanLine()
	{

		String clean = rawLine.replaceAll("\\s", "") ;// Replaces any white
														// space
	 ...
	 * Precondition: cleanLine() has been called POSTCONDITION: Command type is
	 * either: N, A, L, or C
	 */
	private void parseCommandType()
	{
		...
		else if (cleanLine.charAt(0) == '(')// later: check if chartAt(length()
											// - 1) == ')'
		{
			commandType = 'L' ;
		}
	 ...
	 * PRECONDITION: cleanLine() has been called POSTCONDITION: instance
	 * variables for instruction are parsed
	 */
	private void parse()
	{
		parseCommandType() ;
	...
	/**
	 * DESCRIPTION: Helper method that parses into destMnemonic Precondition:
	 * cleanLine() and parseCommandType() have been called POSTCONDITION:
	 * destMnemonic has valid valid value or null
	 */
	...
	/**
	 * DESCRIPTION: Helper method that parses into compMnemonic Precondition:
	 * cleanLine() and parseCommandType() have been called POSTCONDITION:
	 * compMnemonic has valid valid value or null
	 */
	...
	/**
	 * DESCRIPTION: Helper method that parses into jumpMnemonic Precondition:
	 * cleanLine() and parseCommandType() have been called POSTCONDITION:
	 * jumpMnemonic has valid valid value or null
	 */
