 ...
 * 3. Use the CompilationEngine to compile the input JackTokenizer into the output file.â€�
 */
public class JackAnalyzer {
     ...
     * @param dir
     *****************************************************************/
	public JackAnalyzer(String fileInName)
	{
		File fileIn = new File(fileInName) ;
		...
		tokenFileOutPath = fileIn.getAbsolutePath().substring(0, fileIn.getAbsolutePath().lastIndexOf(".")) + "T.xml" ;
		fileOut = new File(fileOutPath) ;
		tokenFileOut = new File(tokenFileOutPath) ;

		CompilationEngine compilationEngine = new CompilationEngine(fileIn, fileOut, tokenFileOut) ;
		compilationEngine.compileClass() ;
