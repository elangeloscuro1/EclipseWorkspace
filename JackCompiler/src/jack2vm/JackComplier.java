...
public class JackComplier {
	 ...
	 * @param dir
	 *****************************************************************/
	public JackComplier(String fileInName)
	{
		File fileIn = new File(fileInName) ;
		String fileOutPath = "" ;
		File fileOut ;
		
		fileOutPath = fileIn.getAbsolutePath().substring(0, fileIn.getAbsolutePath().lastIndexOf(".")) + ".vm" ;
		fileOut = new File(fileOutPath) ;
		
		CompilationEngine compilationEngine = new CompilationEngine(fileIn, fileOut) ;
		compilationEngine.compileClass() ;
